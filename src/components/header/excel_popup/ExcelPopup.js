...
function ExcelPopup({ closePopup, history }) {
  const [
    getEntitySchemaFromGQL,
    { error: getTypeError, loading: getTypeLoading, data: entitySchemaFromGQL }
  ] = useLazyQuery(GET_ENTITY_SCHEMA_FOR_EXCEL);

  const [isSaveDisabled, setIsSaveDisabled] = useState(true);

  const [entityName, setEntityName] = useState("");
  const [
    getAllDataQueryForBuildingExcel,
    setGetAllDataQueryForBuildingExcel
  ] = useState();
  const [allDataQueryName, setallDataQueryName] = useState("");
  const [
    getAllDataQueryForUpdatingState,
    setGetAllDataQueryForUpdatingState
  ] = useState();
  ...
  const covnertEntityGQLSchemaToExcelAcceptedSchema = useCallback(
    entitySchemaFromGQL => {
      ...
      const deleteNonScalarFields = fields => {
        fields = fields.filter(f => f.type.kind === "SCALAR");
        return fields;
      };
      ...
      const convertFieldsFromGqlToExcelAcceptedSchema = fields => {
        ...
        fields.forEach(field => {
          ...
          switch (fieldKind) {
            case "SCALAR":
              schemaForExcel.scalarFieldsNames.push(field.name);
              break;
            ...
            case "OBJECT":
              ...
              schemaForExcel[
                field.name
              ] = convertFieldsFromGqlToExcelAcceptedSchema(
                deleteNonScalarFields(field.type.fields)
              );
              break;

            case "LIST": {
              if (field.type.ofType.kind === "SCALAR") {
                schemaForExcel.scalarsListsNames.push(field.name);
                break;
              }
              ...
              schemaForExcel[
                field.name
              ] = convertFieldsFromGqlToExcelAcceptedSchema(listFields);
      ...
      const { fields } = entitySchemaFromGQL.__type;
      const entitySchemaForExcel = convertFieldsFromGqlToExcelAcceptedSchema(
        fields
      );
  ...
  useEffect(() => {
    if (entitySchemaFromGQL) {
      setEntitySchemaForExcel(
        covnertEntityGQLSchemaToExcelAcceptedSchema(entitySchemaFromGQL)
      );
  ...
  return (
    <div className="excel-popup">
      <div className="close-icon" onClick={closePopup}>
        <img src="icons/clear_grey_27x27.png" alt="..." />
      </div>
      <div className="my-header">
        <span style={{ fontWeight: "bold" }}>{entityName + "s"}</span>
        {" data in excel"}
      </div>
