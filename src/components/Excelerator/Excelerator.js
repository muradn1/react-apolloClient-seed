...
export default function Excelerator() {
  function getCsvHeader(fields) {
    ...
    const listObjects = fields.filter(({ name }) =>
      listNamesInGqlObject.includes(name)
    );
    const nonListObjects = fields.filter(
      ({ name }) => !listNamesInGqlObject.includes(name)
    );
  ...
  function downloadCsvWithData(fileName, data, typeName) {
    const newWorkbook = XLSX.utils.book_new();
    const entry = data[0];
